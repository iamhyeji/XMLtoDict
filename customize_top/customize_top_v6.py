...
def engname(kname):
    name_dict ={
        ...
        "탄력봉" : "Traffic Pole",
        "라바콘" : "Traffic Cone",
        "플라스틱드럼통" : "Traffic Drum",
        "바리케이트" : "Traffic Barricade",
        "일자형볼라드" : "Cylinderical Bollard",
        "U형볼라드" : "U-shaped Bollard",
        "기타도로장벽" : "Other Road Barriers",
        "주차금지표지판" : "No Parking Stand", 
        "주차금지봉" : "Adjustable Parking Pole",
        "폐타이어" : "Waste Tire",
        "도로화분" : "Planter Barrier",
        "물통기름통" : "Water Container",
        "기타이동가능장애물" : "Movable Obstacle",
        "주차차단기몸통" : "Barrier Gate",
        "전기차충전설비" : "Electric Car Charger",
        "주차비정산설비" : "Parking Meter",
        "주차안내표지판" : "Parking Sign",
        "운전자신호등" : "Traffic Light",
        "보행자신호등" : "Pedestrian Light",
        "교통표지" : "Street Sign",
        "장애인주차장" : "Disabled Parking Space",
        "임산부주차장" : "Pregnant Parking Space",
        "전기차주차장" : "Electric Car Parking Space",
        "이륜차주차장" : "Two-wheeled Vehicle Parking Space",
        "기타주차장" : "Other Parking Space",
        "주차가능공간" : "Parking Space",
        "주행가능공간" : "Drivable Space",
        "직각" : "Perpendicular",
        "평행" : "Parallel",
        "대향" : "Angle",
        "교차" : "Herringbone"
...
def newfile(frame,dir_path):
    ...
    new = {
        ...
        "meta" : {
            ...
            'sr' : meta[2],
            'enviroment' : meta[3],
            'weather' : meta[4] if meta[3] == 'outdoor' else "",
            'time':meta[5] if meta[3] == 'outdoor' else "",
            'road_type':meta[-6],
            'state' : meta[-5],
            'place' : meta[-4],
            'name' : meta[-3],
...
def updatefile2(path,track, box, polygon, camera_list,frame):
    file_name = camera_list[frame][:-4]
    with open(f'{path}/pickle/{file_name}.pickle','rb')as f :
        pkl_data = pickle.load(f)
    for data in box,polygon :
        if data is not None and data.attrib['outside']=='0':

            aname = ''.join([i for i in track.attrib['label'] if not i.isdigit()])
            pkl_data['annos']['name']=np.append(pkl_data['annos']['name'],engname(aname))
    ...
    with open(f'{path}/pickle/{file_name}.pickle', 'wb') as make_file:
...
def updatefile3(path,name,object, geometry):
    ...
    aname = ''.join([i for i in object['classTitle'] if not i.isdigit()])
    pkl_data['annos']['name']=np.append(pkl_data['annos']['name'],engname(aname))
...
try:
    ...
    # curr_path = os.path.dirname(os.path.realpath(__file__))  #test
    ...
    for (path,dir,files) in os.walk(curr_path):
        ...
        if task_list :
            ...
            camera_file = glob.glob(f'{path}/*Camera')
            camera_list = [_ for _ in os.listdir(camera_file[0]) if _.endswith(".jpg")]

            for img_name in camera_list :
                newfile(img_name[-12:-4],path)
                ...
                with zipfile.ZipFile(filename) as task :
                    ...
                    for track in root.findall('track'):  
                        for i in range(0,size):
                            ...
                            for data in box,polygon :
                                if data is not None :
                                    frame = int(data.attrib['frame'])+file_num
            
                                    if data.attrib['outside']=='0' :
                                        updatefile2(path,track, box, polygon,camera_list, frame)
                    file_num += size
        ...
        for pickle_file in pickle_list:
            ...
            for i,b in enumerate(data['annos']['bbox']):
                if not np.isnan(b).all() :
                    ...
                    if str(data['annos']['smallcar'][i]) != 'nan' :
                        segm["size"] = "Small" if data['annos']['parked'][i]=='true' else "Normal"
                    if str(data['annos']['pattern'][i]) != 'nan' :
                        segm["pattern"] = engname(data['annos']['pattern'][i])
